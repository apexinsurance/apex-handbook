...
export default defineComponent({
  ...
  mounted() {
    this.updateQuery()
    RegionModule.getAllRegions(this.query)
  },
  methods: {
    handleAddClick() {
      this.$router.push('/region/create')
    },
    ...
    handleDelete(id: number) {
      ElMessageBox.confirm('Вы хотите удалить этот данные?', 'Предупреждение', {
        ...
        .then(async () => {
          await RegionModule.deleteRegion(id)
          RegionModule.getAllRegions(this.query)
          ElMessage({
            type: 'success',
