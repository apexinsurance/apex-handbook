...
import { IUpdateRegionForm } from '@/store/modules/region/region.types'
...
export default defineComponent({
  ...
  async mounted() {
    try {
      ...
      await RegionModule.findRegionById(+this.$route.params.id)
      if (RegionModule.currentRegion) {
        const { code, translations, id, countryId, finishDate } =
          RegionModule.currentRegion
        this.regionForm.code = code
        this.regionForm.id = id
        this.regionForm.countryId = countryId
  ...
  methods: {
    submitForm(formName: string) {
      ;(this.$refs[formName] as any).validate(async (valid: boolean) => {
        if (valid) {
          const { id, code, outdated, countryId, ru, uz, en } = this.regionForm
          const translations = [ru, uz, en] as ITranslatioForm[]
          const formData: IUpdateRegionForm = {
            id,
            code,
            translations,
            countryId,
          ...
          await RegionModule.updateRegion(formData)
          ElMessage({
